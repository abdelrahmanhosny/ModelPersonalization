...
if __name__ == "__main__":
    ...
    print('Sorting out datasets .. ')

    train_dataset = WriterQMNIST()
    test_dataset = WriterQMNIST()
    for writer_id in range(131):
        # leave subject out
        if writer_id == args.writer_id:
            writer_test_ds = torch.load(os.path.join(args.data_dir, 'test', 'w' + str(writer_id) + '.pth' ))
            dataloader = DataLoader(writer_test_ds, batch_size=64)
            for data, label in dataloader:
                test_dataset.add_datapoint(data, label)
        else:
            writer_train_ds = torch.load(os.path.join(args.data_dir, 'train', 'w' + str(writer_id) + '.pth' ))
            dataloader = DataLoader(writer_train_ds, batch_size=64)
            for data, label in dataloader:
                train_dataset.add_datapoint(data, label)
    ...
    torch.save(model.state_dict(), os.path.join(save_dir, 'general-model.pt'))

    with open(os.path.join(save_dir, 'accuracy_before.txt'), 'w') as f:
